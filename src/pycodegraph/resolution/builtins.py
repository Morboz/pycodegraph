...
PYTHON_BUILTINS = frozenset({
    ...
    "bool", "bytes", "bytearray", "frozenset", "object", "complex",
    "open", "input", "type", "isinstance", "hasattr", "getattr", "setattr",
    "super", "self", "cls", "None", "True", "False",
    "next", "iter", "any", "all", "abs", "bin", "chr", "dir", "divmod",
    "enumerate", "eval", "exec", "filter", "format", "frozenset", "globals",
    "hex", "id", "map", "max", "min", "oct", "ord", "pow", "repr", "reversed",
    "round", "sorted", "sum", "vars", "zip",
...
PYTHON_BUILTIN_METHODS = frozenset({
    ...
    "execute", "executemany", "fetchall", "fetchone", "fetchmany", "commit",
    "rollback", "cursor", "connect",
})
...
def _get_python_stdlib_modules() -> frozenset[str]:
    ...
    return _PYTHON_STDLIB_FALLBACK

_PYTHON_STDLIB: Optional[frozenset[str]] = None
...
def is_builtin_or_external(
    ...
    if lang in ("typescript", "javascript", "tsx", "jsx"):
        if name in JS_BUILTINS:
            return True
        if name.startswith("console.") or name.startswith("Math.") or name.startswith("JSON."):
            return True
